...
import { createStorePayment } from "@/app/actions";
import {
  Card,
  Field,
  SelectInput,
...
export function PaymentForm({ stores }: { stores: Store[] }) {
  const [state, action, pending] = useActionState(
    async (_: unknown, formData: FormData) => createStorePayment(formData),
    null,
  );

  return (
    <Card title="Record payment / บันทึกรับชำระ">
      <form action={action} className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Amount (฿)" labelTh="จำนวนที่จ่าย">
          <TextInput
            type="number"
            name="amount_thb"
            min={0.01}
        ...
        <Field label="Payment method" labelTh="ช่องทางจ่าย">
